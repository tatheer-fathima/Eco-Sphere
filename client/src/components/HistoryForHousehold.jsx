  ...
  useEffect(() => {
    ...
    const fetchEntries = async (id) => {
      try {
        ...
        const entriesWithFamily = await Promise.all(
          entriesData.map(async (entry) => {
            try {
              ...
              console.warn(`No family entries for entry ${entry.id}`);
              return { ...entry, familyEntries: [] };
            }
          })
        );
        ...
        console.error("Error fetching household entries:", error);
      }
    };
    ...
    const fetchAllData = async () => {
      try {
        ...
        await Promise.all([fetchEntries(userId), fetchRecommendations(userId)]);
      } catch (error) {
    ...
    <div className="bg-white shadow-lg rounded-lg p-6 space-y-6 mb-28">
      ...
      <div className="flex flex-col gap-10">
        {entries.length > 0 ? (
            ...
            <div key={entryIndex} className="bg-gray-50 p-4 rounded-lg shadow-md space-y-4">
              ...
              >
                <span className="text-gray-600">
                  Sr No. <strong>{entries.length - entryIndex}</strong>
                </span>
                <span className="text-gray-600">
                  Date: <strong>{formatDate(entry.created_at)}</strong>{" "}
                  {expandedEntry === entryIndex ? "▲" : "▼"}
                </span>
              </div>
              ...
              {expandedEntry === entryIndex && (
                <div className="mt-4">
                  <p><strong>Electricity Usage:</strong> {entry.electricity_usage}</p>
                  <p><strong>Water Usage:</strong> {entry.water_usage}</p>
                  <p><strong>Waste Generation:</strong> {entry.waste_generation}</p>
                  <p><strong>Gas Cylinder:</strong> {entry.gas_cylinder}</p>

                  <h3 className="mt-6 text-xl font-semibold">Family Members</h3>
                  {entry.familyEntries && entry.familyEntries.length > 0 ? (
                    <ul className="mt-2 space-y-2">
                      {entry.familyEntries.map((member, memberIndex) => {
                        const memberKey = `${entryIndex}-${memberIndex}`;
                        return (
                          <li key={memberKey} className="bg-gray-100 p-4 rounded-lg shadow-md">
                            <div onClick={() => toggleMember(memberKey)} className="cursor-pointer flex justify-between">
                              <p><strong>Name:</strong> {member.name}</p>
                              <span>{expandedMember === memberKey ? "▲" : "▼"}</span>
                            </div>
                            {expandedMember === memberKey && (
                              <div className="mt-2">
                                <p><strong>Private Vehicle:</strong> {member.private_vehicle}</p>
                                <p><strong>Public Vehicle:</strong> {member.public_vehicle}</p>
                                <p><strong>Air Travel:</strong> {member.air_travel}</p>
                                <p><strong>Veg Meals:</strong> {member.veg_meals}</p>
                                <p><strong>Non-Veg Meals:</strong> {member.non_veg_meals}</p>
                              </div>
                            )}
                          </li>
                        );
                  ...
                  <h3 className="mt-6 text-xl font-semibold text-center">
                    Total Carbon Footprint: {Math.round(carbonFootprints[entry.id] * 100) / 100} KgCO<sub>2</sub>
                  </h3>

                  <AdminSideChart entry={entry} />

                  <h3 className="mt-6 text-xl font-semibold text-center">Recommendations</h3>
                  {recommendations
                    .filter((rec) => rec.household_common_id === entry.id)
                    .map((rec, idx) => (
                      <div
                        key={idx}
                        className="p-4 bg-blue-50 rounded-lg shadow"
                        dangerouslySetInnerHTML={{ __html: formatRecommendations(rec.recommendation) }}
                      />
                    ))}
                </div>
              )}
          ...
          <p className="text-gray-600 text-center">No household data available.</p>
        )}
      </div>
    </div>
  );
